...
function RabitLayout(props) {
  const { layouts } = props;
  const dispatch = useDispatch();
  const settings = useSelector(selectRabitCurrentSettings);
  const defaultSettings = useSelector(selectRabitDefaultSettings);
  ...
  // console.warn('::RabitLayout:: rendered');
...
export default memo(RabitLayout);
