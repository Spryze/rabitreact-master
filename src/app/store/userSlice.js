...
export const UpdateUser = createAsyncThunk(
  ...
  async (formData, { rejectWithValue }) => {
    try {
      ...
      console.log('response of update user', response.data);
...
export const UserProfile = createAsyncThunk('user/UserProfile', async (_, { rejectWithValue }) => {
  try {
    const user = JSON.parse(localStorage.getItem('user'));
    const cont_user_id = user.uid;
...
const userSlice = createSlice({
  ...
  AddUserPropertyAction: (state, action) => {
    console.log('set user property ', action.payload);
    const tempProperties = state.data.properties;

    const index1 = tempProperties.findIndex(
      (property) => property.property_id === action.payload.property_id
    );

    if (index1 !== -1) {
      state.data.properties[index1] = action.payload;
    } else {
      state.data.properties?.push(action.payload);
    }
  },
...
export const { userLoggedOut, AddUserPropertyAction } = userSlice.actions;
